...
class Solution(object):
    ...
    def jump(self, nums):
        ...
        while i < len(nums)-1:
            ...
            # From current position to the current max reach, find next best index and new max reach
            chunk_start = i+1
            chunk_end = min(max_reach, len(nums)-1)
            chunk_best_index, max_reach = self._get_best_index(nums, chunk_start, chunk_end)

            solution.append(str(chunk_best_index))
            i = chunk_end
            # print(max_reach)
            # print(' --- ')
